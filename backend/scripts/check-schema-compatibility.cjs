  ...
  try {
    ...
    for (const metadata of AppDataSource.entityMetadatas) {
      ...
      for (const expected of metadata.columns) {
        const actual = columnByKey.get(`${metadata.tableName}.${expected.databaseName}`);
        if (!actual) {
          failures.push(`${table}.${expected.databaseName}: required column is missing`);
          continue;
        }
        ...
        if (expected.length && Number(expected.length) !== actual.character_maximum_length) {
          failures.push(
        ...
        if (expected.precision !== undefined && expected.precision !== null &&
        ...
        if (expected.scale !== undefined && expected.scale !== null &&
      ...
      for (const expected of metadata.foreignKeys) {
        const expectedColumns = expected.columns.map((column) => column.databaseName);
        const referencedColumns = expected.referencedColumns.map((column) => column.databaseName);
        const match = foreignKeys.find(
          (actual) => actual.table_name === metadata.tableName &&
            actual.referenced_table === expected.referencedEntityMetadata.tableName &&
            actual.columns.join('\u0000') === expectedColumns.join('\u0000') &&
            actual.referenced_columns.join('\u0000') === referencedColumns.join('\u0000'),
        );
        if (!match) {
          failures.push(
