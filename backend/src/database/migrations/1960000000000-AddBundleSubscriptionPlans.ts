...
export class AddBundleSubscriptionPlans1960000000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      ALTER TABLE bundles
        ADD COLUMN IF NOT EXISTS first_plan_enabled boolean NOT NULL DEFAULT false,
        ADD COLUMN IF NOT EXISTS first_plan_price_mcq numeric(10,2),
        ADD COLUMN IF NOT EXISTS first_plan_price_mcq_essay numeric(10,2),
        ADD COLUMN IF NOT EXISTS final_plan_enabled boolean NOT NULL DEFAULT false,
        ADD COLUMN IF NOT EXISTS final_plan_price_mcq numeric(10,2),
        ADD COLUMN IF NOT EXISTS final_plan_price_mcq_essay numeric(10,2);

      DO $$ BEGIN ALTER TABLE bundles ADD CONSTRAINT ck_bundle_first_plan_price CHECK (first_plan_price_mcq IS NULL OR first_plan_price_mcq > 0);
      EXCEPTION WHEN duplicate_object THEN NULL; END $$;
      DO $$ BEGIN ALTER TABLE bundles ADD CONSTRAINT ck_bundle_first_plan_essay_price CHECK (first_plan_price_mcq_essay IS NULL OR first_plan_price_mcq_essay > 0);
      EXCEPTION WHEN duplicate_object THEN NULL; END $$;
      DO $$ BEGIN ALTER TABLE bundles ADD CONSTRAINT ck_bundle_final_plan_price CHECK (final_plan_price_mcq IS NULL OR final_plan_price_mcq > 0);
      EXCEPTION WHEN duplicate_object THEN NULL; END $$;
      DO $$ BEGIN ALTER TABLE bundles ADD CONSTRAINT ck_bundle_final_plan_essay_price CHECK (final_plan_price_mcq_essay IS NULL OR final_plan_price_mcq_essay > 0);
      EXCEPTION WHEN duplicate_object THEN NULL; END $$;
