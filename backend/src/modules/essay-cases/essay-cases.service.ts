...
export class EssayCasesService {
  ...
  async importPdfSeed(courseId:string,actor:AuthenticatedUser){ await this.assertCourseAccess(courseId,actor,true); const weeks:Row[]=await this.db.query(`SELECT id,week_number FROM weeks WHERE course_id=$1 ORDER BY week_number`,[courseId]); const byNumber=new Map(weeks.map(w=>[w.week_number,w.id])); const missing=[...new Set((pdfCases as any[]).map(c=>c.week))].filter(n=>!byNumber.has(n)); if(missing.length) throw new ConflictException(`Create course weeks ${missing.join(', ')} before importing the PDF cases`); let imported=0,skipped=0; await this.db.transaction(async m=>{ for(const source of pdfCases as any[]){ const exists=(await m.query(`SELECT id FROM essay_cases WHERE week_id=$1 AND section=$2 AND source_case_number=$3`,[byNumber.get(source.week),source.section,source.sourceCaseNumber]))[0]; if(exists){skipped++;continue;} await this.insertCase(m,{week_id:byNumber.get(source.week),title:source.title,stem:source.stem,section:source.section,source_case_number:source.sourceCaseNumber,is_published:true,questions:source.questions.map((q:any)=>({prompt:q.prompt,model_answer:q.answer}))},actor.userId); imported++; }}); return {imported,skipped,total:(pdfCases as any[]).length}; }
