...
CREATE TABLE subscription_plans (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(), key varchar(30) NOT NULL,
    label varchar(100) NOT NULL, price_amount numeric(10,2), price_currency varchar(3) NOT NULL DEFAULT 'EGP',
    duration_days integer, created_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
INSERT INTO subscription_plans (key, label, price_amount, duration_days) VALUES
    ('free', 'Free', NULL, NULL), ('first_5_weeks', 'First 5 Weeks', NULL, 35),
    ('last_5_weeks', 'Last 5 Weeks', NULL, 35), ('max', 'Max', NULL, 30);

CREATE TABLE bundle_allowed_plans (
    bundle_id uuid NOT NULL REFERENCES bundles(id) ON DELETE CASCADE,
    plan_id uuid NOT NULL REFERENCES subscription_plans(id) ON DELETE RESTRICT,
    created_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (bundle_id, plan_id)
);
...
CREATE TABLE user_bundle_unlocks (
    ...
    unlocked_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, unlock_reason unlock_reason NOT NULL,
...
CREATE TABLE promo_codes (
    ...
    discount_type promo_discount_type NOT NULL, discount_value numeric(10,2) NOT NULL DEFAULT 0,
    max_uses integer, used_count integer NOT NULL DEFAULT 0, expires_at timestamp,
    is_active boolean NOT NULL DEFAULT true, created_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT uq_promo_codes_code UNIQUE (code),
...
CREATE TABLE promo_code_plans (
    ...
    created_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (promo_code_id, plan_id)
);
...
CREATE TABLE plan_purchases (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    plan_id uuid NOT NULL REFERENCES subscription_plans(id) ON DELETE RESTRICT,
    status plan_purchase_status NOT NULL DEFAULT 'pending', payment_method plan_payment_method NOT NULL,
    amount_paid numeric(10,2) NOT NULL DEFAULT 0, currency varchar(3) NOT NULL DEFAULT 'EGP',
    provider varchar(50), provider_reference varchar(200), provider_order_id varchar(100),
    provider_transaction_id varchar(100), promo_code_id uuid REFERENCES promo_codes(id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX uq_plan_purchases_provider_order ON plan_purchases(provider, provider_order_id)
WHERE provider IS NOT NULL AND provider_order_id IS NOT NULL;
CREATE UNIQUE INDEX uq_plan_purchases_provider_transaction ON plan_purchases(provider, provider_transaction_id)
