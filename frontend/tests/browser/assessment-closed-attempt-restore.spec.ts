...
async function primeAuth(page: Page) {
  await page.addInitScript(() => {
    localStorage.setItem('mdp_access_token', 'closed-attempt-token');
...
for (const status of ['SUBMITTED', 'EXPIRED'] as const) {
  test(`${status.toLowerCase()} assessment restores review directly after refresh`, async ({ page }) => {
    ...
    await page.route('**/*', async (route) => {
      ...
      if (target === '/auth/me') return respond({ id: 'student-closed', email: 'student@example.test', full_name: 'Closed Student', role: 'STUDENT', status: 'ACTIVE', emailVerified: true });
