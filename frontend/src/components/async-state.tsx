...
export function PageSkeleton({ variant = "cards", label = "Loading content" }: { variant?: SkeletonVariant; label?: string }) {
  ...
  return (
    <div className={`shared-skeleton shared-skeleton--${variant}`} role="status" aria-label={label}>
...
export function EmptyState({ title, description, action }: { title: string; description: string; action?: React.ReactNode }) {
  return (
    <section className="shared-state shared-state--empty">
      <FiInbox />
      <h2>{title}</h2>
      <p>{description}</p>
      {action}
...
export function ErrorState({ title = "Something went wrong", description, onRetry }: { title?: string; description: string; onRetry?: () => void }) {
  return (
    <section className="shared-state shared-state--error" role="alert">
      <FiAlertCircle />
      <h2>{title}</h2>
      <p>{description}</p>
      {onRetry && <button className="pp-button secondary" type="button" onClick={onRetry}><FiRefreshCw /> Retry</button>}
    </section>
  );
}
