...
export function RoleDashboardPage() {
  ...
  return <RoleBoundary roles={["INSTRUCTOR", "SYSTEM_ADMIN"]}><ProductShell search={isAdmin ? "Search users or audit records" : "Search courses, questions, or assessments"}><main className="pp-page role-workspace"><WorkspaceHeading eyebrow={isAdmin ? "SYSTEM CONTROL CENTER" : "TEACHING CONTROL CENTER"} title={`Welcome back, ${firstName}`} description={isAdmin ? "Monitor the platform, administer users, and review audit evidence from a focused operations workspace." : "Plan content, publish learning material, assess students, and close the grading loop from one workspace."} actions={<button className="pp-button secondary" type="button" onClick={() => void load()}><FiRefreshCw /> Refresh</button>} />
    ...
    {isAdmin && <div className="role-dashboard-lower"><Panel title="Administrative operating loop"><ol className="role-steps">{["Review platform status", "Create or maintain user accounts", "Resolve access issues", "Review notifications", "Inspect audit evidence"].map((step, index) => <li key={step}><span>{index + 1}</span><div><b>{step}</b><small>{index === 0 ? "Start with the operational overview." : "Continue only within the administrator's operational scope."}</small></div></li>)}</ol></Panel>
      <Panel title="Live backend coverage"><div className="role-coverage"><div><b>Platform operations</b><span>Overview · users · notifications · audit · settings</span></div><div><b>RBAC</b><span>Teaching content remains restricted to instructors.</span></div><div><b>Source</b><span>Every total and list is loaded from current backend records.</span></div></div></Panel></div>}
