...
function StudentDashboardScreen({data,pearl,loading,error,reload}:{data:StudentDashboard|null;pearl:NotebookPage["data"][number]|null;loading:boolean;error:string|null;reload:()=>Promise<void>}) {
  ...
  return <div className="dashboard-shell" data-theme={theme}>
    <aside className={`dash-sidebar ${menu?"open":""}`}><button className="sidebar-close" onClick={()=>setMenu(false)}><FiX/></button><BrandLockup className="dash-brand" href="/dashboard"/>
      <nav><Link className="active" href="/dashboard"><FiHome/>Dashboard</Link><Link href="/bundles"><FiBookOpen/>My Bundles</Link><Link href="/flashcards"><FiActivity/>Review</Link><Link href="/notebook"><FiFileText/>Notebook</Link><Link href="/guidelines"><FiBookOpen/>References</Link></nav>
      ...
      <div className="cme-card"><FiAward/><div><strong>Study resources</strong><span>Continue learning from your assigned bundles.</span><button onClick={()=>router.push("/bundles")}>Explore</button></div></div>
    </aside>
