...
export function ConnectedAssessmentSession({attemptId,testId,source="assessments"}:{attemptId:string;testId:string;source?:string}){
 ...
 const [review,setReview]=useState<Review|null>(null),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState<string|null>(null),[now,setNow]=useState(Date.now());
 ...
 async function choose(optionId:string){if(!current||saving)return;setSaving(true);setError(null);try{const result=await request<Answer&TutorFeedback>(`/tests/attempts/${attemptId}/answers/${current.question.id}`,{method:"PUT",body:{selected_option_id:optionId}});setAnswers(value=>({...value,[current.question.id]:optionId}));if(tutor)setFeedback(value=>({...value,[current.question.id]:{isCorrect:result.isCorrect??null,explanation:result.explanation}}));}catch(cause){setError(cause instanceof Error?cause.message:"Unable to save this answer.");}finally{setSaving(false);}}
 ...
 async function saveNote(){if(!current)return;const value=notes[current.question.id]?.trim()||"";try{if(value)await request(`/tests/attempts/${attemptId}/notes/${current.question.id}`,{method:"PUT",body:{note:value}});else if(notes[current.question.id]!==undefined)await request(`/tests/attempts/${attemptId}/notes/${current.question.id}`,{method:"DELETE"});}catch(cause){setError(cause instanceof Error?cause.message:"Unable to save the note.");}}
 ...
 return <ProductShell><main className={`pp-page mock-page ${tutor?"tutor-workspace":""}`}>{error&&<p className="form-error" role="alert">{error}</p>}{loading?<div className="product-auth-loading">Loading assessment…</div>:review?<Panel title="Assessment submitted"><h2>{review.attempt.test?.title||"Assessment complete"}</h2><p>Your answers and explanations are available below.</p>{review.questions.map((item,i)=><article className="review-answer" key={item.question.id}><b>{i+1}. {item.question.questionText}</b><p>{item.answer?.isCorrect===true?"Correct":item.answer?.isCorrect===false?"Incorrect":"Not answered / pending grading"}</p></article>)}<button className="pp-button" onClick={()=>router.push(exitPath)}>Back</button></Panel>:current?<>
 ...
 <Panel className="question-card"><div className="question-tools"><span className="pp-chip">{tutor?"Tutor mode":"Exam question"}</span><button onClick={()=>void toggleFlag()}><FiFlag/> {flags.includes(current.question.id)?"Unflag":"Flag"}</button></div><h2>{current.question.questionText}</h2><div className="answer-list">{current.question.options.map((option,i)=>{const selected=answers[current.question.id]===option.id;return <button disabled={saving||Boolean(tutor&&feedback[current.question.id])} className={selected?"selected":""} onClick={()=>void choose(option.id)} key={option.id}><b>{String.fromCharCode(65+i)}</b>{option.optionText}{selected&&<FiCheck/>}</button>})}</div>
 ...
 <Panel title="Private question note"><textarea value={notes[current.question.id]||""} onChange={event=>setNotes(value=>({...value,[current.question.id]:event.target.value}))} placeholder="Save a private note for this question…"/><button className="pp-button secondary" onClick={()=>void saveNote()}><FiSave/> Save note</button></Panel></Panel>
 <Panel title={tutor?"Round progress":"Exam Overview"} className="exam-overview"><p>Answered <b>{answered}/{items.length}</b></p><p>Flagged <b>{flags.length}</b></p><Progress value={progress}/><button className="pp-button" disabled={saving} onClick={()=>void submit()}>Finish {tutor?"round":"assessment"}</button></Panel></div>
