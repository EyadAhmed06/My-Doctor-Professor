...
type ReviewOption = Option & { isCorrect?: boolean; explanation?: string | null };
  ...
  return <ProductShell search="Search cases, topics, or concepts"><main className={`pp-page exam-session-page ${tutor ? "is-tutor" : "is-timed"}`}>
    ...
    {loading ? <div className="product-auth-loading">Loading assessment…</div> : error && !attempt ? <Panel title="Could not load assessment"><p>{error}</p><button className="pp-button" type="button" onClick={() => window.location.reload()}>Retry</button></Panel> : review ? <Panel title="Assessment submitted" className="exam-review-panel">
      ...
      {review.questions.map((item, i) => {
        ...
        const reviewFeedback: StructuredTutorFeedback | null = item.question.questionType === "MCQ" && correctOption ? {
          ...
          options: item.question.options.map((option) => ({ id: option.id, explanation: option.explanation })),
        } : null;
        return <article className={`review-answer ${item.answer?.isCorrect === true ? "correct" : item.answer?.isCorrect === false ? "incorrect" : ""}`} key={item.question.id}>
          <b>{i + 1}{forensicCode ? ` · ${forensicCode}` : ""}. {item.question.questionText}</b>
          ...
          {reviewFeedback && <TutorOptionReview feedback={reviewFeedback} options={item.question.options} />}
        </article>;
      })}
