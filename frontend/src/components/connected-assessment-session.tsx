...
export function ConnectedAssessmentSession({attemptId,testId,source="assessments"}:{attemptId:string;testId:string;source?:string}){
 ...
 <Panel className="question-card"><div className="question-tools"><span className="pp-chip">{tutor?"Tutor mode":"Exam question"}</span><button onClick={()=>void toggleFlag()}><FiFlag/> {flags.includes(current.question.id)?"Unflag":"Flag"}</button></div>{questionVisual&&<figure className="question-visual"><img src={questionVisual.src} alt={questionVisual.alt}/></figure>}<h2>{current.question.questionText}</h2><div className="answer-list">{current.question.options.map((option,i)=>{const selected=answers[current.question.id]===option.id;return <button disabled={saving||Boolean(tutor&&feedback[current.question.id])} className={selected?"selected":""} onClick={()=>void choose(option.id)} key={option.id}><b>{String.fromCharCode(65+i)}</b>{option.optionText}{selected&&<FiCheck/>}</button>})}</div>
