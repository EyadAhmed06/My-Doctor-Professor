...
type Candidate = {
  ...
  question_number: number; question_text: string; model_answer: string | null; answer_origin?: "SOURCE" | "AI" | "MISSING"; source_page: number | null;
...
export function EssayQuestionImportPage() {
  ...
  const [candidateSearch,setCandidateSearch]=useState(""); const [statusFilter,setStatusFilter]=useState<"ALL"|Candidate["status"]>("ALL");
  ...
  return <ProductShell search="Search essay imports"><main className="pp-page question-import-page"><header className="question-import-heading"><div><Link className="question-import-back" href={user.role==="SYSTEM_ADMIN"?"/admin/questions":"/instructor/essay-cases"}><FiArrowLeft/>{user.role==="SYSTEM_ADMIN"?"Question bank":"Essay cases"}</Link><span className="page-eyebrow">CASE-BASED ESSAY INGESTION</span><h1>Essay PDF Inspector</h1><p>Detect case stems, numbered essay prompts, and their published answer keys. Nothing is published until you assign the extracted case to a real lecture topic and approve it.</p></div></header>
