...
export function ProductShell({ children, search = "Search cases, topics, or concepts" }: { children: React.ReactNode; search?: string }) {
  ...
  const loadPreview=useCallback(async()=>{
    ...
    try{setPreview((await request<Page<NotificationPreview>>("/notifications?limit=5")).data);}
    catch(cause){notify({title:"Could not load notifications",description:cause instanceof Error?cause.message:undefined,tone:"error"});}
  ...
  useEffect(()=>{
    const refresh=()=>{void loadUnread();if(notificationsOpen)void loadPreview();};
  ...
  useEffect(()=>{
    ...
    function escape(event:KeyboardEvent){
      ...
      if(notificationsOpen){setNotificationsOpen(false);notificationsButtonRef.current?.focus();}
    }
  ...
  async function markPreviewRead(item:NotificationPreview){
    ...
    try{await request(`/notifications/${item.id}`,{method:"PUT"});window.dispatchEvent(new Event("mdp:notifications-changed"));}
    catch{setPreview(current=>current.map(value=>value.id===item.id?{...value,status:"UNREAD"}:value));setUnread(value=>value+1);}
  }
  async function markAllPreviewRead(){
    const previous=preview;
    setPreview(current=>current.map(item=>({...item,status:"READ"})));
    setUnread(0);
    try{await request("/notifications/mark-read",{method:"PUT"});window.dispatchEvent(new Event("mdp:notifications-changed"));}
    catch(cause){setPreview(previous);void loadUnread();notify({title:"Could not mark notifications as read",description:cause instanceof Error?cause.message:undefined,tone:"error"});}
  }
  async function openNotification(item:NotificationPreview){
    await markPreviewRead(item);
    setNotificationsOpen(false);
    if(item.target_url){startNavigation();router.push(item.target_url);}
  }
  ...
  return <div className="product-app">
    <header className="pp-topbar">
      ...
      <nav id="product-navigation" className={open ? "open" : ""} aria-label="Primary navigation"><button className="pp-nav-close" type="button" onClick={()=>setOpen(false)} aria-label="Close menu"><FiX /></button>{nav.map(([label,href])=><Link key={href} className={path.startsWith(href) ? "active" : ""} href={href} onClick={()=>{setOpen(false);startNavigation();}}>{label}</Link>)}{user.role!=="STUDENT"&&<Link href="/instructor/quizzes" onClick={()=>{setOpen(false);startNavigation();}}>Instructor</Link>}<Link href="/settings" onClick={()=>{setOpen(false);startNavigation();}}>Settings</Link></nav>
      ...
      <div className="pp-profile">
        <div className="header-popover-anchor" ref={notificationsRef}>
          <button ref={notificationsButtonRef} aria-label={`${unread} unread notifications`} aria-expanded={notificationsOpen} aria-haspopup="dialog" onClick={()=>{const next=!notificationsOpen;setNotificationsOpen(next);setProfileOpen(false);if(next)void loadPreview();}}><FiBell/>{unread>0&&<i>{unread>99?"99+":unread}</i>}</button>
        ...
        <div className="header-popover-anchor profile-menu-anchor" ref={profileRef}>
          <button ref={profileButtonRef} className="profile-menu-trigger" type="button" aria-expanded={profileOpen} aria-haspopup="menu" onClick={()=>{setProfileOpen(value=>!value);setNotificationsOpen(false);}}><span className="avatar-fallback">{initials}</span><span><b>{displayName}</b><small>{roleLabel}</small></span><FiChevronDown/></button>
