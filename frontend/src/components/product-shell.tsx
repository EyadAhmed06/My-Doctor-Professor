...
export function ProductShell({ children, search = "Search cases, topics, or concepts" }: { children: React.ReactNode; search?: string }) {
  ...
  useEffect(()=>{const refresh=()=>{void loadUnread();if(notificationsOpen)void loadPreview();};window.addEventListener("mdp:notifications-changed",refresh);return()=>window.removeEventListener("mdp:notifications-changed",refresh);},[loadPreview,loadUnread,notificationsOpen]);
  ...
  useEffect(()=>{if(!profileOpen&&!notificationsOpen)return;function outside(event:PointerEvent){const target=event.target as Node;if(profileOpen&&!profileRef.current?.contains(target))setProfileOpen(false);if(notificationsOpen&&!notificationsRef.current?.contains(target))setNotificationsOpen(false);}function escape(event:KeyboardEvent){if(event.key!=="Escape")return;if(profileOpen){setProfileOpen(false);profileButtonRef.current?.focus();}if(notificationsOpen){setNotificationsOpen(false);notificationsButtonRef.current?.focus();}}window.addEventListener("pointerdown",outside);window.addEventListener("keydown",escape);return()=>{window.removeEventListener("pointerdown",outside);window.removeEventListener("keydown",escape);};},[notificationsOpen,profileOpen]);

  async function markPreviewRead(item:NotificationPreview){if(item.status==="READ")return;setPreview(current=>current.map(value=>value.id===item.id?{...value,status:"READ"}:value));setUnread(value=>Math.max(0,value-1));try{await request(`/notifications/${item.id}`,{method:"PUT"});window.dispatchEvent(new Event("mdp:notifications-changed"));}catch{setPreview(current=>current.map(value=>value.id===item.id?{...value,status:"UNREAD"}:value));setUnread(value=>value+1);}}
  async function markAllPreviewRead(){const previous=preview;setPreview(current=>current.map(item=>({...item,status:"READ"})));setUnread(0);try{await request("/notifications/mark-read",{method:"PUT"});window.dispatchEvent(new Event("mdp:notifications-changed"));}catch(cause){setPreview(previous);void loadUnread();notify({title:"Could not mark notifications as read",description:cause instanceof Error?cause.message:undefined,tone:"error"});}}
  async function openNotification(item:NotificationPreview){await markPreviewRead(item);setNotificationsOpen(false);if(item.target_url){startNavigation();router.push(item.target_url);}}
