...
export function ProductShell({ children, search = "Search cases, topics, or concepts" }: { children: React.ReactNode; search?: string }) {
  ...
  return <div className="product-app">
    <header className="pp-topbar">
      ...
      <nav id="product-navigation" className={open ? "open" : ""} aria-label="Primary navigation"><button className="pp-nav-close" type="button" onClick={()=>setOpen(false)} aria-label="Close menu"><FiX /></button>{nav.map(([label,href])=><Link key={href} className={path.startsWith(href) ? "active" : ""} href={href} onClick={()=>setOpen(false)}>{label}</Link>)}{user.role!=="STUDENT"&&<Link href="/instructor/quizzes" onClick={()=>setOpen(false)}>Instructor</Link>}<Link href="/settings" onClick={()=>setOpen(false)}>Settings</Link></nav>
