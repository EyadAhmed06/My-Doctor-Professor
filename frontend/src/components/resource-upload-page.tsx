...
import { FiBookOpen, FiCheckCircle, FiEdit3, FiFile, FiImage, FiRefreshCw, FiTrash2, FiUploadCloud, FiVideo } from "react-icons/fi";
...
export function ResourceUploadPage(){
  ...
  const [dragging,setDragging]=useState(false),[loading,setLoading]=useState(true),[detailLoading,setDetailLoading]=useState(false),[uploading,setUploading]=useState(false),[changingPublication,setChangingPublication]=useState(false),[progress,setProgress]=useState(0),[error,setError]=useState<string|null>(null);
  ...
  const loadCourses=useCallback(async()=>{
    ...
    try{
      ...
      setCourseId(current=>requestedCourseId&&rows.data.some(item=>item.id===requestedCourseId)?requestedCourseId:current||rows.data[0]?.id||"");
    }
  ...
  async function returnSelectedLectureToDraft(){
    if(!selectedLecture||changingPublication)return false;
    setChangingPublication(true);setError(null);
    try{
      const lectureToUpdate=selectedLecture.id;
      await request(`/academic/lectures/${lectureToUpdate}`,{method:"PUT",body:{is_published:false}});
  ...
  async function upload(event:FormEvent){
    ...
    try{
      if(selectedLecture?.isPublished){const ready=await returnSelectedLectureToDraft();if(!ready)return;}
  ...
  return <ProductShell search="Search courses and resources"><main className="pp-page resource-upload-page">
    ...
    {loading?<PageSkeleton variant="workspace" label="Loading resource workspace"/>:<div className="resource-upload-layout">
      <Panel title="Destination" className="resource-destination-panel"><div className="resource-destination-grid"><label>Course<select value={courseId} onChange={event=>setCourseId(event.target.value)}>{courses.map(item=><option key={item.id} value={item.id}>{item.courseCode} · {item.courseName}</option>)}</select></label><label>Lecture<select value={lectureId} onChange={event=>setLectureId(event.target.value)} disabled={detailLoading}>{lectures.map(item=><option key={item.id} value={item.id}>Week {item.week.weekNumber} · {item.lectureNumber}. {item.title}{item.isPublished?" · published":""}</option>)}</select></label></div>{selectedLecture?.isPublished&&<div className="resource-publish-warning"><FiBookOpen/><div><b>Lecture is published</b><p>Uploading will safely return this lecture to draft first. Your selected file and all upload fields will stay exactly as they are.</p></div><button className="pp-button secondary resource-draft-action" type="button" disabled={changingPublication||uploading} onClick={()=>void returnSelectedLectureToDraft()}><FiEdit3/>{changingPublication?"Changing status…":"Return to draft now"}</button></div>}</Panel>
      <Panel title="Upload managed file" className="resource-upload-panel"><form onSubmit={upload} className="resource-upload-form"><div className={`resource-dropzone ${dragging?"dragging":""}`} onDragOver={event=>{event.preventDefault();setDragging(true)}} onDragLeave={()=>setDragging(false)} onDrop={drop} onClick={()=>inputRef.current?.click()} role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")inputRef.current?.click()}}><FiUploadCloud/><b>{file?file.name:"Drop a resource here or browse"}</b><small>{file?`${typeFor(file)} · ${sizeLabel(file.size)}`:"PDF · PNG/JPEG/WebP · MP4/WebM · max 50 MB"}</small><input ref={inputRef} hidden type="file" accept={accept} onChange={browse}/></div><div className="resource-upload-controls"><div className="resource-upload-fields"><label>Resource name<input required maxLength={200} value={name} onChange={event=>setName(event.target.value)}/></label><label>Description<textarea maxLength={5000} rows={5} value={description} onChange={event=>setDescription(event.target.value)}/></label></div>{(uploading||progress>0)&&<div className="resource-upload-progress"><Progress value={progress}/><span>{progress}% {uploading?"uploading":"uploaded"}</span></div>}<button className="pp-button resource-upload-submit" disabled={!file||!lectureId||!name.trim()||uploading||changingPublication}><FiUploadCloud/> {uploading?"Uploading…":selectedLecture?.isPublished?"Return to draft & upload":"Upload resource"}</button></div></form></Panel>
      <Panel title={`Lecture resources · ${resources.length}`} className="resource-library-panel"><div className="resource-managed-list">{resources.length?resources.map(resource=>{const Icon=resource.resourceType==="IMAGE"?FiImage:resource.resourceType==="VIDEO"?FiVideo:FiFile;return <article key={resource.id}><span><Icon/></span><div><b>{resource.resourceName}</b><small>{resource.resourceType} · {resource.mimeType||"external"} · {resource.fileSize?sizeLabel(Number(resource.fileSize)):"size unavailable"}</small><p>{resource.description||resource.originalFilename||"No description"}</p></div><span className="resource-upload-status"><FiCheckCircle/> {resource.uploadStatus}</span><button className="danger" disabled={Boolean(selectedLecture?.isPublished)} onClick={()=>void remove(resource)} title="Delete resource"><FiTrash2/></button></article>;}):<EmptyState title="No resources in this lecture" description="Upload the first managed file above."/>}</div></Panel>
    </div>}
  </main></ProductShell>;
}
