...
export function ForgotPasswordPage(){
  ...
  async function submit(event:FormEvent){event.preventDefault();setLoading(true);setError(null);setMessage(null);try{const result=await apiRequest<{message:string}>("/auth/password/otp/request",{method:"POST",body:{email:email.trim()}});setMessage(result.message);}catch(cause){setError(cause instanceof ApiError?cause.message:"Unable to send the reset code. Check your connection and try again.");}finally{setLoading(false);}}
  const resetHref=`/reset-password?email=${encodeURIComponent(email.trim())}`;
  return <ActionLayout variant="forgot" eyebrow="ACCOUNT RECOVERY" title="Forgot your password?" description="We will verify your inbox with a one-time code, then help you create a new password.">
    <div className={styles.heading}><span className={styles.step}>1</span><div><h2>Recover your account</h2><p>Enter the email associated with your account. We will send you a secure 6-digit reset code.</p></div></div>
    {message?<div className={styles.complete}><span><FiMail/></span><h2>Check your inbox</h2><p>We sent a 6-digit recovery code to <b>{email}</b>. It expires in 10 minutes.</p><Notice message={message} error={null}/><Link className={`primary-button ${styles.submit}`} href={resetHref}>Enter recovery code <icons.Arrow/></Link><button className={styles.secondaryButton} onClick={()=>setMessage(null)}>Use another email</button></div>:
    <form className={styles.form} onSubmit={submit} noValidate><Field label="Email address" icon={icons.Mail} placeholder="you@example.com" type="email" value={email} onChange={e=>setEmail(e.target.value)} required autoComplete="email"/><p className={styles.helper}>For security, we show the same confirmation whether or not an account exists.</p><SubmitButton loading={loading} label="Send reset code"/><Notice message={null} error={error}/></form>}
  </ActionLayout>;
}
...
export function ResetPasswordPage(){
  ...
  async function verifyCode(event:FormEvent){
    ...
    if(!email){setError("This recovery page is missing your email address. Request a new reset code.");return;}
    if(!/^\d{6}$/.test(code)){setError("Enter the complete 6-digit code from your email.");return;}
    setLoading(true);
    try{
      const result=await apiRequest<{message:string;reset_token:string}>("/auth/password/otp/verify",{method:"POST",body:{email,code}});
      setResetToken(result.reset_token);
      setNotice(result.message);
    }catch(cause){
      setError(cause instanceof ApiError?cause.message:"Unable to verify this reset code. Request a new code and try again.");
    }finally{setLoading(false);}
  }

  async function resend(){
    if(!email||cooldown||loading)return;
    setLoading(true);setError(null);setNotice(null);setResetToken("");
    try{
      const result=await apiRequest<{message:string}>("/auth/password/otp/request",{method:"POST",body:{email}});
      setDigits(Array(6).fill(""));
      setNotice(result.message);
      setCooldown(60);
      window.setTimeout(()=>inputs.current[0]?.focus(),0);
    }catch(cause){
      setError(cause instanceof ApiError?cause.message:"Unable to send a new password reset code.");
    }finally{setLoading(false);}
  }

  async function submitPassword(event:FormEvent){
    event.preventDefault();setError(null);setMessage(null);
    if(!resetToken){setError("Verify your recovery code before choosing a new password.");return;}
    if(!checks.length||!checks.upper||!checks.lower||!checks.number){setError("Your password does not meet all security requirements.");return;}
    if(!checks.match){setError("Passwords do not match.");return;}
    setLoading(true);
    try{
      const result=await apiRequest<{message:string}>("/auth/password/reset",{method:"POST",body:{token:resetToken,new_password:password,confirm_password:confirm}});
      setMessage(result.message);
      setNotice(null);
    }catch(cause){setError(cause instanceof ApiError?cause.message:"Unable to reset this password.");}finally{setLoading(false);}
  }

  const title=message?"Password updated.":resetToken?"Create a new password.":"Verify it is you.";
  const description=message?"Your account is secured with your new password.":resetToken?"Choose a strong password you have not used for this account before.":"Enter the one-time recovery code we sent to your inbox.";
  ...
  return <ActionLayout variant="reset" eyebrow="ACCOUNT RECOVERY" title={title} description={description}>
    ...
    resetToken?<form className={styles.form} onSubmit={submitPassword}><Notice message={notice} error={null}/><Field label="New password" icon={icons.Lock} placeholder="Create a secure password" type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={8} autoComplete="new-password"/><Field label="Confirm password" icon={icons.Lock} placeholder="Repeat your password" type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} required minLength={8} autoComplete="new-password"/><div className={styles.requirements}><Check ok={checks.length}>At least 8 characters</Check><Check ok={checks.upper&&checks.lower}>Uppercase and lowercase letters</Check><Check ok={checks.number}>At least one number</Check><Check ok={checks.match}>Passwords match</Check></div><SubmitButton loading={loading} label="Reset password"/><Notice message={null} error={error}/></form>:
    email?<form className={styles.otpForm} onSubmit={verifyCode} noValidate>
      <div className={styles.otpPanel}>
        ...
        <div className={styles.otpMeta}><FiClock/><span>Code expires in 10 minutes. Requesting another code invalidates the previous one.</span></div>
      </div>
      <SubmitButton loading={loading} label="Verify recovery code"/>
      <Notice message={notice} error={error}/>
      <div className={styles.resendRow}><span>No code yet?</span><button type="button" onClick={resend} disabled={loading||cooldown>0}>{cooldown?`Resend in ${cooldown}s`:"Send a new code"}</button></div>
    </form>:<div className={styles.actions}><Notice message={null} error="This recovery page is missing your email address."/><Link className={`primary-button ${styles.submit}`} href="/forgot-password">Request a reset code <icons.Arrow/></Link></div>}
  </ActionLayout>;
}
