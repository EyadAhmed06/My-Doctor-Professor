...
import { FiBarChart2, FiBell, FiBookOpen, FiCalendar, FiClipboard, FiClock, FiFileText, FiLayers, FiPlus, FiSettings } from "react-icons/fi";
import type { IconType } from "react-icons";
import { useUx } from "./ux-provider";
import { readRecentWorkspacePages } from "./workspace-continuity";

type Role="STUDENT"|"INSTRUCTOR"|"SYSTEM_ADMIN";
type Command={label:string;description:string;href:string;icon:IconType;keywords:string[];roles?:Role[];recent?:boolean};
...
export function CommandPalette({open,onOpen,onClose,role}:{open:boolean;onOpen:()=>void;onClose:()=>void;role:Role}){
 const router=useRouter();const {startNavigation}=useUx();const inputRef=useRef<HTMLInputElement>(null);const dialogRef=useRef<HTMLElement>(null);const restoreFocus=useRef<HTMLElement|null>(null);const [query,setQuery]=useState("");const [activeIndex,setActiveIndex]=useState(0);const [recent,setRecent]=useState<Command[]>([]);
 const available=useMemo(()=>{const roleCommands=commands.filter(command=>!command.roles||command.roles.includes(role));const staticHrefs=new Set(roleCommands.map(command=>command.href));return [...recent.filter(command=>!staticHrefs.has(command.href)),...roleCommands];},[recent,role]);
 const filtered=useMemo(()=>{const normalized=query.trim().toLowerCase();if(!normalized)return available;return available.filter(command=>[command.label,command.description,...command.keywords].join(" ").toLowerCase().includes(normalized));},[available,query]);
 useEffect(()=>{const syncRecent=()=>setRecent(readRecentWorkspacePages().map(page=>({label:`Resume ${page.label}`,description:page.href,href:page.href,icon:FiClock,keywords:["recent","history",page.label,page.href],recent:true})));syncRecent();window.addEventListener("mdp:recent-pages-changed",syncRecent);return()=>window.removeEventListener("mdp:recent-pages-changed",syncRecent);},[]);
 useEffect(()=>{function handleGlobalKey(event:KeyboardEvent){if((event.metaKey||event.ctrlKey)&&event.key.toLowerCase()==="k"){event.preventDefault();if(open)onClose();else onOpen();}if(event.key==="Escape"&&open)onClose();}window.addEventListener("keydown",handleGlobalKey);return()=>window.removeEventListener("keydown",handleGlobalKey);},[open,onClose,onOpen]);
 useEffect(()=>{if(!open)return;restoreFocus.current=document.activeElement instanceof HTMLElement?document.activeElement:null;setQuery("");setActiveIndex(0);const frame=window.requestAnimationFrame(()=>inputRef.current?.focus());function trap(event:KeyboardEvent){if(event.key!=="Tab"||!dialogRef.current)return;const focusable=Array.from(dialogRef.current.querySelectorAll<HTMLElement>("button:not([disabled]), input:not([disabled]), [href], [tabindex]:not([tabindex='-1'])"));if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}}window.addEventListener("keydown",trap);return()=>{window.cancelAnimationFrame(frame);window.removeEventListener("keydown",trap);restoreFocus.current?.focus();};},[open]);
 useEffect(()=>{if(activeIndex>=filtered.length)setActiveIndex(Math.max(0,filtered.length-1));},[activeIndex,filtered.length]);
 function execute(command:Command){onClose();startNavigation();router.push(command.href);}
 function handleKeyDown(event:React.KeyboardEvent<HTMLInputElement>){if(event.key==="ArrowDown"){event.preventDefault();setActiveIndex(index=>Math.min(filtered.length-1,index+1));}else if(event.key==="ArrowUp"){event.preventDefault();setActiveIndex(index=>Math.max(0,index-1));}else if(event.key==="Enter"&&filtered[activeIndex]){event.preventDefault();execute(filtered[activeIndex]);}}
 if(!open)return null;
