  ...
  async function createQuestion(event: FormEvent) {
    ...
    try {
      ...
      if (form.question_type === "MCQ") {
        const options = [form.option_a, form.option_b, form.option_c, form.option_d].map(value => value.trim()).filter(Boolean);
        if (options.length < 2) throw new Error("MCQ questions require at least two options.");
        await Promise.all(options.map((option_text, index) => request(`/questions/${created.id}/options`, { method: "POST", body: { option_text, is_correct: index === Number(form.correct_option), display_order: index + 1 } })));
