...
export function QuestionBankPage({ admin = false }: { admin?: boolean }) {
  ...
  const [removedEditOptionIds, setRemovedEditOptionIds] = useState<string[]>([]);
  ...
  async function createQuestion(event: FormEvent) {
    ...
    try {
      ...
      if (form.question_type === "MCQ") {
        ...
        ].slice(0, form.option_count);
        if (options.some(option => !option.text.trim() || !option.explanation.trim())) throw new Error("All selected options and their explanations are required.");
        if (Number(form.correct_option) >= options.length) throw new Error("Choose a correct answer from the available options.");
  ...
  return <RoleBoundary><ProductShell search="Search questions and topics"><main className="pp-page role-workspace"><Heading eyebrow={admin ? "ADMIN · CONTENT CONTROL" : "INSTRUCTOR · QUESTION BANK"} title="Question bank" description="Create complete MCQ and essay records, reuse existing questions, and control whether each item is available for assessment building." actions={<><button className="pp-button secondary" onClick={() => void load()} type="button"><FiRefreshCw /> Refresh</button><button className="pp-button" onClick={() => setCreateOpen(true)} type="button"><FiPlus /> New question</button></>} />
    ...
    <Modal title={`Edit ${editingQuestion?.questionType === "MCQ" ? "MCQ" : "essay question"}`} open={Boolean(editingQuestion)} onClose={() => { if (!saving) { setEditingQuestion(null); setRemovedEditOptionIds([]); } }} wide><form className="role-form" onSubmit={saveQuestionEdit}><div className="role-form-grid"><label>Title<input value={editQuestionForm.title} onChange={event => setEditQuestionForm(current => ({ ...current, title: event.target.value }))} /></label><label>Difficulty<select value={editQuestionForm.difficulty} onChange={event => setEditQuestionForm(current => ({ ...current, difficulty: event.target.value as "EASY" | "MEDIUM" | "HARD" }))}><option value="EASY">Easy</option><option value="MEDIUM">Medium</option><option value="HARD">Hard</option></select></label><label>Marks<input required type="number" min="0.01" step="0.01" value={editQuestionForm.marks} onChange={event => setEditQuestionForm(current => ({ ...current, marks: Number(event.target.value) }))} /></label><label className="wide">Question text<textarea required rows={5} value={editQuestionForm.question_text} onChange={event => setEditQuestionForm(current => ({ ...current, question_text: event.target.value }))} /></label><label className="wide">General explanation<textarea rows={4} value={editQuestionForm.explanation} onChange={event => setEditQuestionForm(current => ({ ...current, explanation: event.target.value }))} /></label>{editingQuestion?.questionType === "MCQ" ? <div className="wide case-question-editor"><div className="role-hero-actions"><button className="pp-button secondary" type="button" disabled={editOptions.length >= 5} onClick={addEditOption}><FiPlus /> Add choice</button><small>{editOptions.length} choice{editOptions.length === 1 ? "" : "s"} · four or five supported</small></div>{editOptions.map((option, index) => <Panel key={option.id || `new-option-${index}`} title={`Option ${String.fromCharCode(65 + index)}`}><label className="case-publish"><input type="radio" name="correct-option" checked={option.isCorrect} onChange={() => setEditOptions(items => items.map((item, itemIndex) => ({ ...item, isCorrect: itemIndex === index })))} /> Correct answer</label><label>Choice text<textarea required rows={2} value={option.optionText} onChange={event => setEditOptions(items => items.map((item, itemIndex) => itemIndex === index ? { ...item, optionText: event.target.value } : item))} /></label><label>Choice explanation<textarea rows={3} value={option.explanation || ""} onChange={event => setEditOptions(items => items.map((item, itemIndex) => itemIndex === index ? { ...item, explanation: event.target.value } : item))} /></label><button className="pp-button secondary" type="button" disabled={editOptions.length <= 4} onClick={() => removeEditOption(index)}><FiTrash2 /> Remove choice</button></Panel>)}</div> : <><label className="wide">Model answer<textarea required rows={6} value={editQuestionForm.model_answer} onChange={event => setEditQuestionForm(current => ({ ...current, model_answer: event.target.value }))} /></label><label className="wide">Grading rubric<textarea required rows={5} value={editQuestionForm.grading_rubric} onChange={event => setEditQuestionForm(current => ({ ...current, grading_rubric: event.target.value }))} /></label></>}</div><footer><button className="pp-button secondary" type="button" disabled={saving} onClick={() => { setEditingQuestion(null); setRemovedEditOptionIds([]); }}>Cancel</button><button className="pp-button" disabled={saving} type="submit">{saving ? "Saving…" : "Save all changes"}</button></footer></form></Modal>
