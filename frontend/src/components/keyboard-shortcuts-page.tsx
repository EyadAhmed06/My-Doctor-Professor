...
] satisfies Array<{ title: string; icon: typeof FiCommand; rows: string[][] }>;

const arabicGroupTitles: Record<string,string> = {
  "Global navigation": "التنقل العام",
  "Command palette": "لوحة الأوامر",
  "Learning workspaces": "مساحات التعلم",
};
...
export function KeyboardShortcutsPage() {
  ...
  return <ProductShell search={locale === "ar" ? "ابحث في الأوامر والصفحات" : "Search commands and pages"}><main className="pp-page shortcut-manual-page">
    ...
    <div className="shortcut-group-grid">{groups.map(group => { const Icon = group.icon; return <Panel key={group.title} title={locale === "ar" ? arabicGroupTitles[group.title] || group.title : group.title}><div className="shortcut-group-icon"><Icon /></div><dl className="shortcut-list">{group.rows.map(([keys, description]) => <div key={keys}><dt><kbd>{keys}</kbd></dt><dd>{description}</dd></div>)}</dl></Panel>; })}</div>
